...
/* --- PaintTerrainMask ---
 * Queue a brush operation that writes to the albedo/mask channel instead of height. */
bool TerrainManipulator::PaintTerrainMask(LandscapeLayerMapPtr lmap, ImagePtr mask_image, float opacity)
{
    if (!lmap || !mask_image)
        return false;
...
void UiPanelTerrain::setupUI()
{
    ...
    // --- Mask Paint ---
    auto* mask_group = new QGroupBox("Mask Paint", this);
    auto* mask_layout = new QVBoxLayout(mask_group);

    auto* mask_row = new QHBoxLayout();
    mask_row->addWidget(new QLabel("Mask Layer:"));
    combo_mask_name_ = new QComboBox(this);
    combo_mask_name_->setToolTip("Select a landscape mask/detail layer to paint on");
    mask_row->addWidget(combo_mask_name_);
    btn_refresh_masks_ = new QPushButton("Refresh", this);
    btn_refresh_masks_->setMaximumWidth(70);
    connect(btn_refresh_masks_, &QPushButton::clicked, this, &UiPanelTerrain::onRefreshMaskList);
    mask_row->addWidget(btn_refresh_masks_);
    mask_layout->addLayout(mask_row);

    auto addMaskSpinRow = [&](const QString& label, double min_val, double max_val,
                              double default_val, double step, const QString& tip) -> QDoubleSpinBox*
    {
        auto* row = new QHBoxLayout();
        row->addWidget(new QLabel(label));
    ...
    spin_paint_opacity_ = addMaskSpinRow("Paint Opacity:", 0.0, 1.0, 1.0, 0.1,
        "Opacity of the mask paint (0=transparent, 1=solid)");
    spin_paint_hardness_ = addMaskSpinRow("Paint Hardness:", 0.0, 1.0, 0.8, 0.1,
        "Edge hardness of the mask paint (0=soft, 1=hard)");

    btn_paint_mask_ = new QPushButton("Apply Paint To Mask", this);
    btn_paint_mask_->setStyleSheet("QPushButton { background-color: #7f5f2a; color: white; padding: 10px; font-weight: bold; }");
    connect(btn_paint_mask_, &QPushButton::clicked, this, &UiPanelTerrain::onApplyPaintMask);
    mask_layout->addWidget(btn_paint_mask_);

    main_layout->addWidget(mask_group);

    // Populate mask dropdown on init
 ...
 * Enumerate detail mask layers from the active ObjectLandscapeTerrain and fill QComboBox.
 * Uses ObjectLandscapeTerrain::getNumDetailMasks() / getDetailMask(int) API. */
void UiPanelTerrain::populateMaskDropdown()
{
    using namespace Unigine;

    combo_mask_name_->clear();

    ObjectLandscapeTerrainPtr terrain = Landscape::getActiveTerrain();
    if (!terrain)
    {
        combo_mask_name_->addItem("(no terrain)");
        return;
    }

    int num_masks = terrain->getNumDetailMasks();
    for (int d = 0; d < num_masks; d++)
    {
        TerrainDetailMaskPtr mask = terrain->getDetailMask(d);
        QString name = QString("Detail Mask %1").arg(d);
        if (mask)
        {
            const char* mask_name = mask->getName();
            if (mask_name && mask_name[0] != '\0')
                name = QString::fromUtf8(mask_name);
        }
        combo_mask_name_->addItem(name);
    }

    if (num_masks == 0)
    {
        // Fallback: add generic albedo entry so the button is usable
        combo_mask_name_->addItem("Mask 0 (albedo)");
    }
}

void UiPanelTerrain::onRefreshMaskList()
{
    populateMaskDropdown();
    logMessage("Mask list refreshed.");
}

/* --- Apply Paint To Mask ---
 * Paint the selected mask/albedo layer on terrain tiles intersecting selected meshes. */
void UiPanelTerrain::onApplyPaintMask()
{
    using namespace Unigine;
    using namespace Unigine::Math;

    logMessage("=== Paint Terrain Mask ===");
    progress_bar_->setValue(0);
    ...
    QString mask_name = combo_mask_name_->currentText();
    int mask_index = combo_mask_name_->currentIndex();
    float paint_opacity = static_cast<float>(spin_paint_opacity_->value());
    float paint_hardness = static_cast<float>(spin_paint_hardness_->value());

    logMessage(QString("Mask: '%1' (index %2), Opacity: %3, Hardness: %4")
        .arg(mask_name).arg(mask_index).arg(paint_opacity).arg(paint_hardness));
    ...
    for (auto& node : selected)
    {
        ...
        // Find surface to use for mask coverage area
        ...
        for (int t = 0; t < terrain->getNumChildren(); t++)
        {
            ...
            // Create mask paint image (RGBA8 for albedo channel)
            ImagePtr mask_image = Image::create();
            mask_image->create2D(tile_res.x, tile_res.y, Image::FORMAT_RGBA8);

            // Initialize with transparent black (no paint)
            Image::Pixel zp;
            zp.i.r = 0; zp.i.g = 0; zp.i.b = 0; zp.i.a = 0;
            for (int x = 0; x < tile_res.x; x++)
                for (int y = 0; y < tile_res.y; y++)
                    mask_image->set2D(x, y, zp);

            // Rasterize the mesh footprint onto the mask image
            auto iworld = lmap->getIWorldTransform();
            auto res = lmap->getResolution();
            auto size = lmap->getSize();
            auto step = size / Vec2(res.x, res.y);

            Unigine::Vector<dvec3> verts_ws;
            appendBaseVerts(obj_surface, verts_ws);

            bool any_painted = false;
            int paint_alpha = Math::clamp(static_cast<int>(255.0f * paint_opacity), 0, 255);

            // Hardness controls the falloff from full alpha at center to edge
            // hardness=1 means sharp edges, hardness=0 means very soft edges
            for (int i = 2; i < (int)verts_ws.size(); i += 3)
            {
                ...
                for (int x = mn_x; x <= mx_x; x++)
                {
                    ...
                    for (int y = mn_y; y <= mx_y; y++)
                    {
                        ...
                        {
                            Image::Pixel mp;
                            mp.i.r = 255; mp.i.g = 255; mp.i.b = 255;
                            mp.i.a = paint_alpha;
                            mask_image->set2D(x, y, mp);
                            any_painted = true;
                        }
                    }
                }
            ...
            if (any_painted)
            {
                // Apply soft edge falloff based on hardness
                if (paint_hardness < 1.0f)
                {
                    int falloff_pixels = std::max(1, static_cast<int>((1.0f - paint_hardness) * 20.0f));
                    // Simple box-blur pass for soft edges
                    ImagePtr blurred = Image::create();
                    blurred->create2D(tile_res.x, tile_res.y, Image::FORMAT_RGBA8);
                    // Copy mask_image to blurred
                    for (int x = 0; x < tile_res.x; x++)
                        for (int y = 0; y < tile_res.y; y++)
                            blurred->set2D(x, y, mask_image->get2D(x, y));
                    for (int pass = 0; pass < falloff_pixels; pass++)
                    {
                        for (int x = 1; x < tile_res.x - 1; x++)
                        {
                            for (int y = 1; y < tile_res.y - 1; y++)
                            {
                                auto c  = mask_image->get2D(x, y);
                                auto l  = mask_image->get2D(x - 1, y);
                                auto r  = mask_image->get2D(x + 1, y);
                                auto u2 = mask_image->get2D(x, y - 1);
                                auto d  = mask_image->get2D(x, y + 1);
                                Image::Pixel avg;
                                avg.i.r = (c.i.r + l.i.r + r.i.r + u2.i.r + d.i.r) / 5;
                                avg.i.g = (c.i.g + l.i.g + r.i.g + u2.i.g + d.i.g) / 5;
                                avg.i.b = (c.i.b + l.i.b + r.i.b + u2.i.b + d.i.b) / 5;
                                avg.i.a = (c.i.a + l.i.a + r.i.a + u2.i.a + d.i.a) / 5;
                                blurred->set2D(x, y, avg);
                            }
                        }
                        // Copy blurred back to mask_image
                        for (int x = 0; x < tile_res.x; x++)
                            for (int y = 0; y < tile_res.y; y++)
                                mask_image->set2D(x, y, blurred->get2D(x, y));
                    }
                }

                if (plugin_->getManipulator()->PaintTerrainMask(lmap, mask_image, paint_opacity))
                    logMessage(QString("  Painted mask on tile %1").arg(t));
                else
                    logMessage(QString("  WARNING: Failed to paint tile %1 (brush material missing)").arg(t));
            }
            else
            {
                logMessage(QString("  No mesh coverage on tile %1, skipping").arg(t));
            }
        }
    ...
    logMessage("=== Mask Paint Complete ===");
}
